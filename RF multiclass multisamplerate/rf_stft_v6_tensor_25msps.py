...
#        I_stft = torch.stft(torch.tensor(I), n_fft=w, hop_length=int(w*.75), win_length=w1, window=win, center=True, normalized=True, onesided=False)
#        Q_stft = torch.stft(torch.tensor(Q), n_fft=w, hop_length=int(w*.75), win_length=w1, window=win, center=True, normalized=True, onesided=False)        
...
#                path_name = '/home/david/sigMF_ML/RF/train_val_25msps/25msps_val_b205'
                path_name = './dataset_temp'
                for fname in os.listdir(path_name):
                    if fname.endswith('.sigmf-meta') and fname.startswith(real_fname):
                        ...
                        copyfile(path_name+'/{}.sigmf-meta'.format(real_fname),
    ...
    def plot_IQ_initial(self, iq_plot):
        ...
        self.ax.imshow(iq_plot, vmin=-70, vmax=5, aspect='auto', origin='lower') # matches validation segmented loader
