...
class signal_boxing_GUI_main(QWidget):
    ...
    def gpu(self, db, n_fft):
        ...
        I_stft = torch.stft(torch.tensor(I), n_fft=n_fft, hop_length=n_fft//2, win_length=w, window=win, center=True, normalized=True, onesided=False)
        Q_stft = torch.stft(torch.tensor(Q), n_fft=n_fft, hop_length=n_fft//2, win_length=w, window=win, center=True, normalized=True, onesided=False)
        ...
        Z_stft = torch.cat((Z_stft[n_fft//2:,:,:],Z_stft[:n_fft//2,:,:]))
        print('Z shape =', Z_stft.shape)
