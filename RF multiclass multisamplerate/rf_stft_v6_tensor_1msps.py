    ...
    def export_raw_pressed(self):
        if self.all_bounded.__len__() == 0:
            ...
            for idx, bound in enumerate(self.all_bounded):
                ...
                if not os.path.exists('./data_temp1'):
                    os.mkdir('./data_temp1')

                if not os.path.exists('./data_temp1/{}'.format(real_fname)):
                    os.mkdir('./data_temp1/{}'.format(real_fname))

                if not os.path.exists('./data_temp1/{}/sigmf-data'.format(real_fname)):
                    os.mkdir('./data_temp1/{}/sigmf-data'.format(real_fname))

                data_folder_path = './data_temp1/{}/sigmf-data'.format(real_fname)

                fullpath = './data_temp1/{}/sigmf-data/bound_{}_{}'.format(real_fname, str(idx), time_stamp)
                IQ_bounded = bound['IQ_bounded']
                full = {'bounded': IQ_bounded}
                with open(fullpath + '.pickle', 'wb') as f:
                    pickle.dump(full, f)

                for fname in os.listdir('./dataset_temp1'):
                    if fname.endswith('.sigmf-meta') and fname.startswith(real_fname):

                        if not os.path.exists('./data_temp1/{}/sigmf-meta'.format(real_fname)):
                            os.mkdir('./data_temp1/{}/sigmf-meta'.format(real_fname))

                        copyfile('./dataset_temp1/{}.sigmf-meta'.format(real_fname),
                                 './data_temp1/{}/sigmf-meta/{}.sigmf-meta'.format(real_fname, real_fname))
