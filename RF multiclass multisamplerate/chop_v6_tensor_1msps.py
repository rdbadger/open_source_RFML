...
class load_pic_v2_2(QtWidgets.QWidget):
    ...
    def readFilesPressed(self):
        # read multiple files
        self.fnames= []
        self.fnames = QFileDialog.getOpenFileNames(self, 'Open Files', './data_temp1')
        self.items = []
        ...
        for idx, item in enumerate(self.items):
            ...
            eps = 1e-15
            ax.pcolormesh(20*np.log10(np.abs(temp+eps)))
            layout.addWidget(canvas)
            canvas.draw()
            split = fname.split('/')
            self.fn = split[-1]
            self.data_folder_name = split[-3]
            if not os.path.exists('./data_temp1/{}/bound-plots'.format(self.data_folder_name)):
                os.mkdir('./data_temp1/{}/bound-plots'.format(self.data_folder_name))

            plt.savefig('./data_temp1/{}/bound-plots/{}.png'.format(self.data_folder_name, self.fn))
    ...
    def buttonPressed(self):
        ...
        for idx, item in enumerate(chopped):
            ...
            if chop.shape[0] == row and chop.shape[1] == col:
                path = './data_temp1/{}/chopped-data-{}-{}'.format(self.data_folder_name, self.widthText.text(),
