...
class signal_boxing_GUI_main(QWidget):
    ...
    def gpu(self, db, n_fft):
        ...
        den = 2
        w1 = n_fft
        win = torch.hann_window(w1, periodic=True, dtype=None, layout=torch.strided, requires_grad=False)
        I_stft = torch.stft(torch.tensor(I), n_fft=w, hop_length=w//den, win_length=w1, window=win, center=True, normalized=True, onesided=False)
        Q_stft = torch.stft(torch.tensor(Q), n_fft=w, hop_length=w//den, win_length=w1, window=win, center=True, normalized=True, onesided=False)
    ...
    def iq_read(self, file):
        ...
        eps = 1e-15
        self.stft_gpu =  20*np.log10(np.abs(stft_gpu[:,:,0]+eps))
    ...
    def export_raw_pressed(self):
        if self.all_bounded.__len__() == 0:
            ...
            for idx, bound in enumerate(self.all_bounded):
                ...
                if not os.path.exists('./data/{}'.format(real_fname)):
                    os.mkdir('./data/{}'.format(real_fname))

                if not os.path.exists('./data/{}/sigmf-data'.format(real_fname)):
                    os.mkdir('./data/{}/sigmf-data'.format(real_fname))

                data_folder_path = './data/{}/sigmf-data'.format(real_fname)

                fullpath = './data/{}/sigmf-data/bound_{}_{}'.format(real_fname, str(idx), time_stamp)
                ...
                for fname in os.listdir('./dataset'):
                    if fname.endswith('.sigmf-meta') and fname.startswith(real_fname):
                        ...
                        if not os.path.exists('./data/{}/sigmf-meta'.format(real_fname)):
                            os.mkdir('./data/{}/sigmf-meta'.format(real_fname))

                        copyfile('./dataset/{}.sigmf-meta'.format(real_fname),
                                 './data/{}/sigmf-meta/{}.sigmf-meta'.format(real_fname, real_fname))
    ...
    def onselectIQ(self, eclick, erelease):
        ...
        eps = 1e-15
        self.ax_bounded[0].pcolormesh(10*np.log10(np.abs(self.IQ_bounded[:,:,0]+eps)))
